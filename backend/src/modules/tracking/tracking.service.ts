...
export class TrackingService {
  ...
  async saveActivity(
    ...
    if (!normalizedData.routePoints || normalizedData.routePoints.length === 0) {
    ...
    const computedDistance = this.calculateRouteDistance(normalizedData.routePoints);
    ...
    if (normalizedData.capturedHexIds && normalizedData.capturedHexIds.length > 0) {
      normalizedData.territoriesCaptured = normalizedData.capturedHexIds.length;
    }

    // Derive average speed from duration if possible
    if (normalizedData.startTime && normalizedData.endTime && normalizedData.distanceMeters) {
      const start = new Date(normalizedData.startTime);
      const end = new Date(normalizedData.endTime);
      const durationSeconds = Math.max(0, (end.getTime() - start.getTime()) / 1000);
      if (durationSeconds > 0) {
        normalizedData.averageSpeed = normalizedData.distanceMeters / durationSeconds;
      }
    }

    // Derive points from distance (1 point per 100m)
    if (normalizedData.distanceMeters) {
      normalizedData.pointsEarned = Math.round(normalizedData.distanceMeters / 100);
    }
    ...
    await this.userService.updateStats(userId, {
      distanceKm: normalizedData.distanceMeters ? normalizedData.distanceMeters / 1000 : 0,
    ...
    const streakDate = normalizedData.endTime ? new Date(normalizedData.endTime) : new Date();
    await this.userService.updateStreak(userId, streakDate);
  ...
  private normalizeActivityData(
    ...
    const routePoints = activityData.routePoints?.map((point: any) => ({
      ...
      altitude:
        typeof point.altitude === 'number' ? point.altitude : undefined,
    }));
  ...
  private calculateRouteDistance(
    routePoints: Array<{ latitude: number; longitude: number; timestamp?: Date }>,
  ): number {
